...
{
    public static float[] Solve(float[,] matrix, float[] d)
    {
        ...
        if (rows != cols || rows != len)
        {
            ...
            throw new Exception("Matrix is not square or vector size does not match matrix size.");
        }
...
internal sealed class CubicSplineInterpolation
{
    ...
    public float? Interpolate(float p)
    {
        if (baseset)
        {
            ...
            if (H != null)
            {
                ...
                float[] solution = Tridiagonal.Solve(H, dD3);
    ...
    private static float[,] Diag(float[,] matrix, float[] diagVals)
    {
        int rows = matrix.GetLength(0);
        int cols = matrix.GetLength(1);
